document.addEventListener("DOMContentLoaded", function () {
      ...
      if (this.readyState === 4) {
        if (this.status === 200) {
          ...
          data.forEach(function (gv) {
            ...
            row.insertCell(0).textContent = gv.Tai_khoan;
            row.insertCell(1).textContent = gv.Mat_khau;
            row.insertCell(2).textContent = gv.Phan_quyen;
          });
        } else {
    ...
    xhttp.open("GET", "get_nguoidung.php", true);
    xhttp.send();
  });
